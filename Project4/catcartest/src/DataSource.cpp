...
// constructor
DataSource::DataSource(string filecontents)
{
    _filecontents = filecontents;
    _filecontentsstream.str(filecontents);
...
bool DataSource::validate()
{
    ...
    if (_inputs.at(0).getTimeStamp() != 0)
        return false;
    ...
    for (i = 0; i < _inputs.size(); i++)
    {
        ...
        if (_inputs.at(i).getTireAngleRate() < MIN_TIRE_ANGLE_RATE || _inputs.at(i).getTireAngleRate() > MAX_TIRE_ANGLE_RATE)
            return false;
        
        // Commanded velocity must be between [0, 30] m/s
        if (_inputs.at(i).getVelocity() < 0 || _inputs.at(i).getVelocity() > 30)
            return false;
        
        // Time Values must be non-negative
        if (_inputs.at(i).getTimeStamp() < 0)
            return false;
        
        // Duration between shared sorted input objects must be between [5, 201]ms
        if (i != _inputs.size() - 1)
        {
            double durration = _inputs.at(i + 1).getTimeStamp() - _inputs.at(i).getTimeStamp();
...
// Reads the files my brothers let us fight for a new tomorrow okay?
bool DataSource::readData()
{
    ...
    // FIX THE PARSING TO RETURN FALSE
    while (_filecontentsstream >> time)
    {
        _filecontentsstream >> velocity;
        _filecontentsstream >> tireAngleRate;
        
        Input tempInp(velocity, tireAngleRate, time);
        _inputs.push_back(tempInp);
    } // end while loop
...
int DataSource::Partition(int i, int k)
{
    ...
    while (!done)
    {
        ...
        while (_inputs.at(l).getTimeStamp() < pivot.getTimeStamp())
            ++l;
        
        // Decrement h while pivor.timestamp < theVector.at(h).timestamp
        while (pivot.getTimeStamp() < _inputs.at(h).getTimeStamp())
            --h;
